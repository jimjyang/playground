...
__In addition to the properties that are specified here, the properties in the class Data service (dcat:DataService) specified in https://data.norge.no/specification/dcat-ap-no#Datatjeneste[DCAT-AP-NO ↗, window="_blank", role="ext-link"] (in Norwegian) may also be used.__
|===
