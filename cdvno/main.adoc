...
// image:images/warning-playground.png[WIP,72,72] Det som utformes/utprøves i denne sandkassa er: Forslag til metadatamodell i "Spesifikasjon for innholdsbeskrivelse" (arbeidstittel), forkortet til CDV-NO (Content Description Vocabulary - Norwegian specification). 

[cols="10,90", grid=nome,frame=none] 
|===
| image:images/warning-playground.png[playground,200,200] | Det som utformes/utprøves i denne sandkassa er: Forslag til metadatamodell i "Spesifikasjon for innholdsbeskrivelse" (arbeidstittel), forkortet til CDV-NO (Content Description Vocabulary - Norwegian specification). 
|===
...
* Obligatorisk/skal (_mandatory/shall_) betyr "skal alltid tas med"; anbefalt/bør (_recommended/should_) betyr "bør tas med hvis finnes" og valgfri/kan (_optional/may_) betyr "kan tas med". 
* Et gitt innhold som skal "tagges" (= beskrives med metadata) er en instans av klassen "Innhold (rdfs:Recource)".
* Innholdet kan nås via en instans av klassen "Distribusjon (dcat:Distribution)" eller en instans av klassen "Datatjeneste (API) (dcat:DataService)".
...
* Her er link:files/test-nb.html[HTML-visning] av en demo-instans av klassen "Block (cdvno:Block)" og dens distribusjon. 
* Her er link:files/test.ttl[RDF Turtle-representasjon] av demo-instansen av klassen "Block (cdvno:Block)" og dens distribusjon.  
...
#_Egenskapene er forsøkt sortert etter norske navn._# 
