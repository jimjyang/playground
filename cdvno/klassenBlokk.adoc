...
<<img-klassenBlokk>> viser klassen Blokk (cdvno:Block) og klassen den refererer til. Det som er #gulet# ut, er avvik fra https://data.norge.no/specification/dcat-ap-no#Datasett[DCAT-AP-NO sin klasse Datasett (dcat:Dataset) ↗, window="_blank", role="ext-link"]. 
